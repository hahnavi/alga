...
func (ICSRoleAssignment) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("role_type").Values("incident_commander", "communications_lead", "responder").Default("responder"),
		field.Enum("status").Values("active", "ended").Default("active"),
		field.Enum("assignee_type").Values("user", "agent").Default("user"),
		field.Text("scope_description").Optional().Nillable(),
		field.Enum("ended_reason").Values("replaced", "incident_resolved", "assigned", "agent_offline").Optional().Nillable(),
