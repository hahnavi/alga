...
func (_q *AlertQuery) QueryIncidents() *IncidentQuery {
...
// QueryAlertInvestigationAlerts chains the current query on the "alert_investigation_alerts" edge.
func (_q *AlertQuery) QueryAlertInvestigationAlerts() *AlertInvestigationAlertQuery {
...
// QueryDeliveryTargets chains the current query on the "delivery_targets" edge.
func (_q *AlertQuery) QueryDeliveryTargets() *DeliveryTargetQuery {
	query := (&DeliveryTargetClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AlertQuery) WithIncidents(opts ...func(*IncidentQuery)) *AlertQuery {
	query := (&IncidentClient{config: _q.config}).Query()
...
// WithAlertInvestigationAlerts tells the query-builder to eager-load the nodes that are connected to
// the "alert_investigation_alerts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AlertQuery) WithAlertInvestigationAlerts(opts ...func(*AlertInvestigationAlertQuery)) *AlertQuery {
	query := (&AlertInvestigationAlertClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAlertInvestigationAlerts = query
	return _q
}
...
// WithDeliveryTargets tells the query-builder to eager-load the nodes that are connected to
// the "delivery_targets" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AlertQuery) WithDeliveryTargets(opts ...func(*DeliveryTargetQuery)) *AlertQuery {
	query := (&DeliveryTargetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Alert.Query().
//		GroupBy(alert.FieldFingerprint).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *AlertQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Alert, error) {
	...
	if query := _q.withIncidents; query != nil {
		if err := _q.loadIncidents(ctx, query, nodes,
			func(n *Alert) { n.Edges.Incidents = []*Incident{} },
			func(n *Alert, e *Incident) { n.Edges.Incidents = append(n.Edges.Incidents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAlertInvestigationAlerts; query != nil {
		if err := _q.loadAlertInvestigationAlerts(ctx, query, nodes,
			func(n *Alert) { n.Edges.AlertInvestigationAlerts = []*AlertInvestigationAlert{} },
			func(n *Alert, e *AlertInvestigationAlert) {
				n.Edges.AlertInvestigationAlerts = append(n.Edges.AlertInvestigationAlerts, e)
			}); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withDeliveryTargets; query != nil {
...
func (_q *AlertQuery) loadIncidents(ctx context.Context, query *IncidentQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *Incident)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(alert.IncidentsTable)
		s.Join(joinT).On(s.C(incident.FieldID), joinT.C(alert.IncidentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(alert.IncidentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(alert.IncidentsPrimaryKey[1]))
...
func (_q *AlertQuery) loadAlertInvestigationAlerts(ctx context.Context, query *AlertInvestigationAlertQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *AlertInvestigationAlert)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(alertinvestigationalert.FieldAlertID)
	}
	query.Where(predicate.AlertInvestigationAlert(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(alert.AlertInvestigationAlertsColumn), fks...))
	}))
...
func (_q *AlertQuery) loadDeliveryTargets(ctx context.Context, query *DeliveryTargetQuery, nodes []*Alert, init func(*Alert), assign func(*Alert, *DeliveryTarget)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Alert)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
