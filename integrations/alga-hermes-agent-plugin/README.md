...
| **31 agent tools** | `alga_resolve_alert`, `alga_reopen_alert`, `alga_promote_to_incident`, `alga_set_outcome`, `alga_cancel_investigation`, `alga_pause_investigation`, `alga_search_knowledge`, `alga_get_knowledge`, `alga_create_knowledge`, `alga_list_alerts`, `alga_triage_feedback`, `alga_set_incident_priority`, `alga_set_incident_severity`, `alga_trigger_escalation`, `alga_dispatch_task`, `alga_complete_task`, `alga_list_tasks`, `alga_synthesize_findings`, `alga_mitigate_incident`, `alga_resolve_incident`, `alga_set_incident_resolution_docs`, `alga_begin_triage`, `alga_promote_incident`, `alga_add_incident_timeline`, `alga_assign_incident_role`, `alga_get_incident_context`, `alga_get_incident_timeline`, `alga_post_handoff`, `alga_publish_status_update`, `alga_list_services`, `alga_who_is_on_call` |
...
Coordination is now **task-driven**. The commander decomposes incidents into typed tasks via `alga_dispatch_task` (kinds: `investigate`, `communicate`, `verify`, `mitigate`). Dispatched tasks wake the assigned agent through the `coordination_task_dispatched` SSE event; the agent acts on the task and completes it via `alga_complete_task` with a typed result. The commander tracks progress with `alga_list_tasks` and writes the incident conclusion with `alga_synthesize_findings`. This replaces the legacy `alga_request_status_update` (removed) and the deprecated `alga_post_handoff` coordination flow.

| Active incident role | Allowed incident actions |
|---|---|
| Incident commander | Priority, escalation, mitigation, resolution, resolution documents, triage/promote, role assignment, task dispatch/track/synthesize (`alga_dispatch_task`, `alga_list_tasks`, `alga_synthesize_findings`) |
| Responder or assigned investigator | Investigation thread updates, severity, outcome, pause/cancel investigation, milestone status updates, complete dispatched investigate/verify/mitigate tasks (`alga_complete_task`) |
| Communications lead | Publishing public status updates and completing dispatched communicate-kind tasks (`alga_complete_task`) |

> Resolution requires five structured artifacts — `summary`, `impact_assessment`, `actions_taken`, `root_cause`, and `resolution`. The `root_cause` and `resolution` sections are incident document cards shown on the incident page and are independently mandatory (actions_taken no longer satisfies resolution). A commander supplies them inline to `resolve_incident` or stages them with `set_incident_resolution_docs`, then resolves. The commander can also dispatch a communicate-kind task (`alga_dispatch_task` with `kind=communicate`) to ask the communicator to publish a status update, and synthesizes the final conclusion with `alga_synthesize_findings`. `alga_get_incident_context`, `alga_list_alerts`, and `alga_add_incident_timeline` require the `investigate` capability and are denied to a pure commander token — ask the responder for that information instead.
...
Because of that, the goal is not to silence interruptions (e.g. do **not** set Hermes `display.busy_input_mode: queue` for Alga agents — it would also queue `/stop` and make runaway processes hard to abort). The goal is to avoid *unnecessary* ones:

- Activate a role with its dedicated tool, not an extra @mention. A commander activates the communicator by dispatching a communicate-kind task (`alga_dispatch_task` with `kind=communicate`), which sends the task directly and wakes the communicator via the `coordination_task_dispatched` event — the commander should not also post a separate `@communicator` coordination message. A responder publishes status directly with `alga_publish_status_update` rather than delegating to the communicator.
...
| `alga_dispatch_task` | Commander-only. Dispatch a typed coordination task to a role (replaces `alga_post_handoff` and the removed `alga_request_status_update`). Use `kind=investigate` for technical work (auto-creates a child investigation), `kind=communicate` to request a public status update from the communicator, `kind=verify`, or `kind=mitigate`. |
| `alga_complete_task` | Complete a coordination task assigned to you with a typed result (investigate: finding/hypothesis_confidence/evidence/root_cause_candidate; communicate: channel/message_ref; verify; mitigate). |
| `alga_list_tasks` | List coordination tasks for an incident (commander tracks dispatched task progress). Supports optional `status`/`assignee_role`/`limit` filters. GET `/api/v1/agent/incidents/{id}/tasks`. |
| `alga_synthesize_findings` | Commander-only. Synthesize findings from completed child investigations and tasks into the incident's coordinating investigation summary. Call after child tasks complete to write the incident conclusion. |
