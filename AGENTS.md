...
- Use store helpers from `apps/backend/store/pg_helpers.go` (`pgctx`/`pgctxLong`, `handleQueryErr`, `pgStoreBase`, duplicate-key helpers, limit/skip extraction, sort parsing). Transactions go through Bun `RunInTx` (auto-rollback).
